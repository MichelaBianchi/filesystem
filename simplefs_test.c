...
// Dimensione libera FirstFileBlock
...
// Funzione ausiliaria per verificare che il file non esista già nella dir corrente
	...
	for(i = 0; i < space; i++)
		if(file_blocks[i] > 0 && (DiskDriver_readBlock(disk, &ffb_aux, file_blocks[i])) != -1)
			if(!strncmp(ffb_aux.fcb.name, filename, 128))
				return i; // Ritorna il blocco i-esimo
	return -1;
	//o -1 se non esiste
}
...
DirectoryHandle* SimpleFS_init(SimpleFS* fs, DiskDriver* disk){
	...
	// Assegno il disco al FS
	fs->disk = disk;
	
	FirstDirectoryBlock* fdb = malloc(sizeof(FirstDirectoryBlock));
	
	// Leggo il blocco 0 per la root, se c'è lascio perdere
	...
	// Se no creo l'handle e lo restituisco
...
FileHandle* SimpleFS_createFile(DirectoryHandle* d, const char* filename){
	...
	// quindi creo un nuovo DirectoryBlock a cui fare riferimento
	...
	if(!found){
		...
		// Prendo un blocco libero sulla quale allocare il nuovo db
...
int SimpleFS_readDir(char** names, DirectoryHandle* d){
	...
	if (fdb->num_entries > 0){
		...
		// Devo verificare se ci sono file anche in altri DB
...
int SimpleFS_changeDir(DirectoryHandle* d, char* dirname){
	...
	if(!strncmp(dirname, "..", 2)){
		...
		d->pos_in_block = 0; // reset posizione assoluta
		d->dcb = d->directory; // pointer primo blocco al padre
		...
		printf("ERRORE: la directory è vuota\n");
		return -1;
	}
...
int SimpleFS_read(FileHandle* f, void* data, int size){
	...
	int pos_cursore = f->pos_in_file; // posizione del cursore
...
// QUI INIZIA IL MAIN PER TESTARE
